    ...
    var fail = function(id, schema, obj, params)
    {
        ...
        switch(id)
        {
            ...
            case 'errNoMatchedValue':
                return returnFalse(typeof(obj) + " does not match any of the given values ["+(schema.Name?schema.Name+": ":"")+"'"+JSON.stringify(obj)+"']");
            case 'errMatchedNotValue':
                return returnFalse(typeof(obj) + " matches one of the NotValues ("+schema.NotValues[i]+") ["+(schema.Name?schema.Name+": ":"")+"'"+JSON.stringify(obj)+"']");
